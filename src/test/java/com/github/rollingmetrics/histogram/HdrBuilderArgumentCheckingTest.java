...
import static com.github.rollingmetrics.histogram.HdrBuilder.MAX_CHUNKS;
import static com.github.rollingmetrics.histogram.HdrBuilder.MIN_CHUNK_RESETTING_INTERVAL_MILLIS;
...
public class HdrBuilderArgumentCheckingTest {
    ...
    public void nullExecutorShouldBeDeprecated() {
        new HdrBuilder().withBackgroundExecutor(null);
    }

    @Test
    public void validateResetByChunksParametersTest() {
        new HdrBuilder().resetReservoirPeriodicallyByChunks(Duration.ofMillis(HdrBuilder.MIN_CHUNK_RESETTING_INTERVAL_MILLIS * HdrBuilder.MAX_CHUNKS), HdrBuilder.MAX_CHUNKS);
        try {
            new HdrBuilder().resetReservoirPeriodicallyByChunks(Duration.ofMillis(-1), 2);
            fail("should disallow negative duration");
        } catch (IllegalArgumentException e) {}

        try {
            new HdrBuilder().resetReservoirPeriodicallyByChunks(Duration.ofMillis(HdrBuilder.MIN_CHUNK_RESETTING_INTERVAL_MILLIS - 1), HdrBuilder.MAX_CHUNKS);
            fail("should disallow too short duration");
        } catch (IllegalArgumentException e) {}

        try {
            new HdrBuilder().resetReservoirPeriodicallyByChunks(Duration.ofMillis(HdrBuilder.MIN_CHUNK_RESETTING_INTERVAL_MILLIS), HdrBuilder.MAX_CHUNKS + 1);
            fail("should too many chunks");
        } catch (IllegalArgumentException e) {}

        try {
            new HdrBuilder().resetReservoirPeriodicallyByChunks(Duration.ofMillis(HdrBuilder.MIN_CHUNK_RESETTING_INTERVAL_MILLIS), 0);
