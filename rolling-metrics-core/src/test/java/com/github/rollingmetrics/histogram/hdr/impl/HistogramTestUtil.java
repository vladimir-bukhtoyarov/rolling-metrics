...
public class HistogramTestUtil {

    public static void runInParallel(RollingHdrHistogram histogram, long durationMillis) throws InterruptedException {
        ...
        for (int i = 0; i < threads.length; i++) {
            threads[i] = new Thread(() -> {
                try {
                    // update histogram 100 times and take snapshot on each cycle
                    while (errorRef.get() == null && System.currentTimeMillis() - start < durationMillis) {
                        for (int j = 1; j <= 10; j++) {
                            histogram.update(ThreadLocalRandom.current().nextInt(j));
                        }
